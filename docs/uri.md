# RaidenCash URIs
Represents a RaidenCash payment URI. RaidenCash URI strings is a good standard to share payment request, sometimes as a RaidenCash link or using a QR code.
...
## URI Validation
The main use that we expect you'll have for the `URI` class in RaidenCashcore is validating and parsing RaidenCash URIs. A `URI` instance exposes the address as a RaidenCashcore `Address` object and the amount in satoshis, if present.
