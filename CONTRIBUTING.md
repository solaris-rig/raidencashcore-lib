# Contributing to raidencashcore-lib

We're working hard to make *raidencashcore-lib* the most powerful JavaScript library for working with RaidenCash. Our goal is to have *raidencashcore-lib* be a library that can be used by anyone interested in RaidenCash, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.raidencash.io/forum (Support Forum)
...
These are some global design goals in raidencashcore-lib that any change must adhere.
...
When possible, raidencashcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of raidencashcore-lib methods. For example, calling `JSON.stringify` on an raidencashcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'raidencashcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the raidencashcore object.

DO:
```javascript
var raidencashcore = require('../');
var PublicKey = raidencashcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add dashevo git@github.com:solaris-rig/raidencashcore-lib.git
...
```
Finally go to [github.com/solaris-rig/raidencashcore-lib](https://github.com/solaris-rig/raidencashcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of raidencashcore-lib.

If you have any questions feel free to post them to
[github.com/solaris-rig/raidencashcore-lib/issues](https://github.com/solaris-rig/raidencashcore-lib/issues).
